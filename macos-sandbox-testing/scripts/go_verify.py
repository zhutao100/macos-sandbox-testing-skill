#!/usr/bin/env python3
"""Verify macos-sandbox-testing is active in a Go module.

This runs `go test ./...` with `SEATBELT_SANDBOX_SELFTEST=1`, then checks for:
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Verify macos-sandbox-testing is active (go test)")
    ap.add_argument("--project-root", required=True, type=Path, help="Go module root containing go.mod")
    ap.add_argument(
        "--allow-test-failure",
        action="store_true",
        help="Do not fail verification if `go test` exits non-zero; still require sandbox artifacts to exist.",
    )
    ap.add_argument("--go", default="go", help="go executable to use (default: go)")
    ap.add_argument("--go-args", nargs=argparse.REMAINDER, help="Extra args passed to `go test`")
    args = ap.parse_args()

    project_root: Path = args.project_root.resolve()
    if not (project_root / "go.mod").exists():
        print(f"ERROR: go.mod not found under {project_root}", file=sys.stderr)
        sys.exit(2)
    ...
    # Disable Go's test result cache so the bootstrap actually runs.
    cmd = [args.go, "test", "-count=1", "./..."]
    if args.go_args:
    ...
    if test_failed:
        ...
        if not args.allow_test_failure:
            print(f"ERROR: go test failed with exit code {proc.returncode}", file=sys.stderr)
            sys.exit(proc.returncode)
        print(
            f"WARN: go test failed with exit code {proc.returncode}; continuing to verify sandbox artifacts.",
            file=sys.stderr,
        )
    ...
    if test_failed:
        print(f"WARN: go test exited non-zero ({proc.returncode}), but sandbox appears active.", file=sys.stderr)
