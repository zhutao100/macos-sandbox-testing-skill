#!/usr/bin/env python3
"""Verify macos-sandbox-testing is active in a SwiftPM package.

This runs `swift test` with SEATBELT_SANDBOX_SELFTEST=1, then checks that the
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Verify macos-sandbox-testing is active")
    ...
    if "macos-sandbox-testing" not in pkg_text:
        print(
            "WARN: Package.swift does not appear to be patched by macos-sandbox-testing (no markers found).",
            file=sys.stderr,
        )
        print(
    ...
    # Locate sandbox root.
    sandbox_parent = package_root / ".build" / "macos-sandbox-testing"
